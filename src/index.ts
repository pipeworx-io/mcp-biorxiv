...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'bioRxiv / medRxiv');
}
...
const tools: McpToolExport['tools'] = [
  {
    // fleet #1329: bare `details` sits in the catalog next to get_game_details,
    // yt_video_details, auction_lot_details and ~27 other *_details tools and
    // is the only one meaning bioRxiv — a model has no way to attribute it.
    // The gateway only auto-namespaces on a cross-pack COLLISION (see
    // NAME_PACKS in workers/gateway/src/index.ts); this name is unique, so it
    // was left bare. `biorxiv_details` below is now the model-facing name;
    // this bare entry is a permanent alias to the same handler, kept because
    // it carries live traffic (26 calls/30d) — do not remove it.
    name: 'details',
    description: 'DEPRECATED ALIAS for biorxiv_details — identical behavior, kept for existing callers. New callers should use biorxiv_details.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description: 'bioRxiv / medRxiv preprint metadata by DOI or by date / date range.',
    inputSchema: {
      type: 'object',
      properties: {
        server: { type: 'string', description: 'biorxiv | medrxiv' },
  ...
  {
    // Same fleet #1329 alias pattern as details above.
    name: 'published',
    description: 'DEPRECATED ALIAS for biorxiv_published — identical behavior, kept for existing callers. New callers should use biorxiv_published.',
    inputSchema: {
      type: 'object',
      properties: {
        server: { type: 'string' },
  ...
  {
    name: 'biorxiv_published',
    description: 'bioRxiv / medRxiv preprints that have since been published in a peer-reviewed journal.',
    inputSchema: {
      type: 'object',
      properties: {
        server: { type: 'string' },
  ...
  {
    // Same fleet #1329 alias pattern.
    name: 'publisher',
    description: 'DEPRECATED ALIAS for biorxiv_publisher — identical behavior, kept for existing callers. New callers should use biorxiv_publisher.',
    inputSchema: {
      type: 'object',
      properties: {
        server: { type: 'string' },
  ...
  {
    name: 'biorxiv_publisher',
    description: 'bioRxiv / medRxiv preprints subsequently published by a given DOI prefix (e.g. "10.1038" for Nature).',
    inputSchema: {
      type: 'object',
      properties: {
        server: { type: 'string' },
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
    // fleet #1329: biorxiv_details / biorxiv_published / biorxiv_publisher are
    // the new model-facing (source-carrying) names; the bare names are
    // permanent deprecated aliases to the same handler, kept because they
    // carry live traffic. Never diverge the two.
    case 'details':
    case 'biorxiv_details': {
